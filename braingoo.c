...
void print_cl(int argc, char** argv){
...
int get_cl_args(int argc, char** argv, int num_possible_args, ...){
    ...
    int max_loop = min(argc, num_possible_args+1);
...
void print_usage(){
    printf("Usage: ./braingoo <-compile | -interp> <output filename> <input filename> <-k (keep transpilation file)>\n");
}
...
int main(int argc, char** argv){
    ...
    if(!strncmp(compiling_str, "-compile", 9)){
        ...
        printf("Interpreting! Also, implement me!!\n");
    }
